...
1 4 kick 0.5";
  ...
  positions=[0]++str.findAllRegexp("^ [0-9]").collect{|m|m[0][0]};
  chunks=positions.collect {|from,i|
    var to = p[i+1];
    to = if(to.isNil){nil}{to-1};
    str[from..to];
  };
  block {
    // Order of parallel blocks is undefined, they
    // come at the same time anyway so may be written in any order
    var bag = nil;
    chunks.do{|chunk|
      var lines;
      lines = chunk.split($\n).collect{|l|l.stripWhiteSpace}
      if (chunk[0] = $  && chunk[1] != $  ) {
        if (bag.isNil) {
          bag = Bag[];
        };
